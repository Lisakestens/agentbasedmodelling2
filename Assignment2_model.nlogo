...
undirected-link-breed[twoways twoway]
...
to update-links-local-number-passed
  let max-neighbor-passes max [number-of-aircraft-passed] of link-neighbors
  if length sort link-neighbors > 0
  [foreach sort link-neighbors [ neighbor ->
    if max-neighbor-passes > 0 [
    ask link who [who] of neighbor [
      set weight 1 + [number-of-aircraft-passed] of neighbor / max-neighbor-passes]
    ]
  ]
  ]
  ...
  let aircrafts-waiting aircrafts in-radius 30 with [free = "false" and last-infra != nobody]
    ...
    foreach sort aircrafts-waiting [ aircraft-waiting ->
      ...
      if length saturated-path > 0 [
        foreach saturated-path [link-in-path ->
          ask link-in-path [
          set weight 1 + [waiting-time] of aircraft-waiting / max [waiting-time] of aircrafts-waiting ;+ (position link-in-path saturated-path + 1 )/ (length saturated-path )
        ]
      ]
    ]
  ]
...
to update-weights
  ...
  if Weight-updates = "Local-number-of-passed-aircraft" [
  ...
  if other-aircraft-1 != nobody
    [
            ...
            if [patch-x] of other-aircraft-1 = following-patch-x and [patch-y] of other-aircraft-1 = following-patch-y ;and ([free] of other-aircraft-1) = false
                [set free "false"]
;           if [following-patch-x] of other-aircraft-1 = following-patch-x and [following-patch-y] of other-aircraft-1 = following-patch-y and ([heading] of other-aircraft-1 - heading = 270 or [heading] of other-aircraft-1 - heading = -90) and (any? othet aircraft
;        [set free "true" set [free] of other-aircraft-1 "false"]
;            if [patch-x] of other-aircraft-1 = following-patch-x and [patch-y] of other-aircraft-1 = following-patch-y ;and ([free] of other-aircraft-1) = false
;                [set free "false"]
...
;  if any? aircrafts-on patch-left-and-ahead 45 2 and
;    [set free "true"]
;  if any? aircrafts-on patch-ahead 2
;    [set free "true"]
...
;    if x_inf = [pxcor] of neighbor and y_inf > [pycor] of neighbor
;    [set aircrafts-on-link [who] of aircrafts with[ xcor = x_inf   and  ycor < y_inf and ycor > [pycor] of neighbor ]
;    ifelse length aircrafts-on-link > 0 [
;     ifelse  [heading] of item 0 aircrafts-on-link = 0 [
;          ask link [who] of neighbor who [die]
;        ]
;
;
;     ]
